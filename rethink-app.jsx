...
async function callClaude(messages, system, maxTokens = 800, { timeoutMs = 25000, retries = 1 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Retry on rate limit / transient server errors, not on bad request
...
// Extracts the first valid JSON value from a text blob, tolerant of
// markdown fences, leading/trailing prose, and minor trailing commas.
// Calls Claude with the server-side web_search tool enabled. This is what
// makes custom-company lookups actually grounded rather than free recall:
// Claude decides when it needs current info, the search runs on
// Anthropic's infrastructure, and real source URLs come back in the
// response. We extract both the final text AND the citation list so the
// UI can show "this fact came from this URL" rather than asking the user
// to blindly trust it.
async function callClaudeWithWebSearch(userQuery, system, maxTokens = 1500, { timeoutMs = 35000, retries = 1 } = {}) {
  let lastErr;
  for (let attempt = 0; attempt <= retries; attempt++) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeoutMs);
    try {
      ...
      const d = await res.json();
      // Pull out final text blocks and any citations attached to them
      ...
      // Also catch web_search_tool_result blocks for source URLs even if
      // citations weren't attached inline to the text block.
      const searchResults = d.content?.filter(b => b.type === "web_search_tool_result") || [];
      searchResults.forEach(r => {
        (r.content || []).forEach(item => {
          if (item.url && !citations.find(x => x.url === item.url)) {
            citations.push({ url: item.url, title: item.title || item.url });
          }
        });
      });
      const searchedAtAll = (d.content || []).some(b => b.type === "server_tool_use" && b.name === "web_search");
      if (!text) throw new Error("EMPTY_RESPONSE");
      return { text, citations, searchedAtAll };
    } catch (e) {
      clearTimeout(timer);
      lastErr = e.name === "AbortError" ? new Error("TIMEOUT") : e;
...
async function callClaudeWithWebSearchJSON(userQuery, system, maxTokens = 1500, opts = {}) {
  const { text, citations, searchedAtAll } = await callClaudeWithWebSearch(userQuery, system, maxTokens, opts);
  try {
...
async function callClaudeJSON(messages, system, maxTokens = 1000, opts = {}) {
  const text = await callClaude(messages, system, maxTokens, opts);
  try {
...
// Simple in-memory cache keyed by a string — avoids redundant API calls
// for identical requests within the same session (e.g. re-opening a
// company brief you already generated).
const _cache = new Map();
async function callClaudeJSONCached(cacheKey, messages, system, maxTokens, opts) {
  if (_cache.has(cacheKey)) return { data: _cache.get(cacheKey), fromCache: true };
  const data = await callClaudeJSON(messages, system, maxTokens, opts);
...
Return ONLY valid JSON, no markdown, no explanation.`;
      ...
      const { data, citations: cites, searchedAtAll } = await callClaudeWithWebSearchJSON(`Research and build a PM interview brief for: ${trimmed}`, sys, 1800, { retries: 1 });

      if (!searchedAtAll) {
        // Claude answered without actually searching — this is exactly the
        // hallucination risk we're trying to eliminate. Don't show the
        // result as if it were grounded; force a retry that requires search.
        throw new Error("NO_SEARCH_PERFORMED");
      }
      ...
      if (e.message === "NO_SEARCH_PERFORMED") {
        setErrorMsg(`Claude answered without searching the web first, so we discarded the result rather than risk showing you ungrounded facts. Try again — this usually resolves on retry.`);
      } else {
        setErrorMsg(friendlyError(e));
      }
    }
        ...
        {mode === "custom" && (
          <>
            ...
            <InfoBox type="warning">
              🔎 <strong>Live web search, not memory:</strong> when you search a custom company, Claude is required to actually search the web in real time — every fact must trace to a real source URL shown below the brief. If the result isn't well-sourced, we'll tell you honestly rather than guess. Less reliable than the 6 pre-verified companies, but far safer than free-recall AI.
            </InfoBox>
          </>
        )}
